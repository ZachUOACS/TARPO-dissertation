...
def preprocess_dapo(split="train", chunk_size=1000, root='data/DAPO') -> Dataset:
    problems, solutions = [], []
    ...
    for file in os.listdir(split_dir):
        if file.endswith('.json'):
            ...
            prompt_text = entry.get('prompt', entry.get('problem', ''))
            problems.append(prompt_text)
    ...
    return dataset.map(process_dapo, batched=True, 
                       batch_size=chunk_size, load_from_cache_file=False)


def main(args):
    bias = args.action_bias
    exp_name = (f"./experiments/{args.model_name.split('/')[-1]}-dapo-math-tarpo-group{args.group_size}"
    ...
    dataset = preprocess_dapo('train', chunk_size=500, root=args.dataset_root)
    trainer = GRPOTrainer(
        model = model,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset_root", type=str, default="data/DAPO")
